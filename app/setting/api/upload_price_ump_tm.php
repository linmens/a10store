                ...
                switch ($case) {
                    case "导入促销价_天猫":
                        ...
                        if ($data->sheets[0]['cells'][$i][$j] === "最终价") {
                            $goods_price_ump_tm_j = $j;//促销价_天猫
                        }
                        if ($data->sheets[0]['cells'][$i][$j] === "淘宝Id") {
                            $num_iid_tm_j = $j;//宝贝ID_天猫
                        }
                        if ($data->sheets[0]['cells'][$i][$j] === "宝贝状态") {
                            $state_onsale_tm_j = $j;//宝贝状态
                        }
                        break;
                }
            } else {
                    ...
                    case "导入促销价_天猫":
                        switch ($j) {
                            case $outer_id_ds_j://产品编号_电商
                                ...
                                break;
                            case $goods_price_ump_tm_j://促销价_淘宝
                                $goods_price_ump_tm = $data->sheets[0]['cells'][$i][$j];
                                break;
                            case $num_iid_tm_j://宝贝ID_天猫
                                $num_iid_tm = $data->sheets[0]['cells'][$i][$j];
                                break;
                            case $state_onsale_tm_j://宝贝状态
                                $state_onsale_tm = $data->sheets[0]['cells'][$i][$j];
        ...
        if ($i != 1) {
            switch ($case) {
                case "导入促销价_天猫":
                    ...
                    $re_data .= '"goods_price_ump_tm":"' . $goods_price_ump_tm . '",';
                    $re_data .= '"num_iid_tm":"' . $num_iid_tm . '",';
                    $re_data .= '"state_onsale_tm":"' . $state_onsale_tm . '"}';
                    break;
            }
        }
    }
    ...
    switch ($case) {
        case "导入促销价_天猫":
            foreach ($data_arr as $vv) {
                ...
                $goods_price_ump_tm = $vv['goods_price_ump_tm'];
                $num_iid_tm = $vv['num_iid_tm'];
                $state_onsale_tm = $vv['state_onsale_tm'];

                $up = "UPDATE `tbapi_yibu_goods_info` SET ";
                $up .= "`num_iid_tm` = '$num_iid_tm',";
                $up .= " `goods_price_ump_tm` = '$goods_price_ump_tm', ";
                $up .= " `state_onsale_tm` = '$state_onsale_tm' ";
