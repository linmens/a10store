...
//获取今年数据
    $qs_this_year = "SELECT `date_time_int`,`shop_yibu_tm`,`shop_yibu_tb`,`shop_yibu_yhd` FROM `tbapi_yibu_sales_info` WHERE `date_time_int` > 20170100 AND `date_time_int` < $time_now ORDER BY `date_time_int` ASC";
    $data_arr_jn = $ein->e_mysql_search($qs_this_year);

    $qs_last_year = "SELECT `date_time_int`,`shop_yibu_tm`,`shop_yibu_tb`,`shop_yibu_yhd` FROM `tbapi_yibu_sales_info` WHERE `date_time_int` > 20160100 AND `date_time_int` < 20170100 ORDER BY `date_time_int` ASC";
    ...
    foreach ($data_arr_jn as $v1) {
        ...
        foreach ($data_arr_qn as $v2) {
            ...
            if ($date_time_int_qn == $date_time_int_jn2) {
                ...
                $shop_yibu_last_year = $shop_yibu_tm_last_year + $shop_yibu_tb_last_year + $shop_yibu_yhd_last_year;//依布今年-数据


                //返回 总销售额 依布 天猫 去年
                $all_sales_yb_tm_qn = $all_sales_yb_tm_qn + $shop_yibu_tm_last_year;
                $all_sales_yb_tm_qn = round($all_sales_yb_tm_qn, 0);

                //返回 总销售额 依布 淘宝 去年
                $all_sales_yb_tb_qn = $all_sales_yb_tb_qn + $shop_yibu_tb_last_year;
                $all_sales_yb_tb_qn = round($all_sales_yb_tb_qn, 0);

                //返回 总销售额 依布 一号店 去年
                $all_sales_yb_yhd_qn = $all_sales_yb_yhd_qn + $shop_yibu_yhd_last_year;
                ...
                $list_yb_tm_qn .= $shop_yibu_tm_last_year;
                ...
                $list_yb_tb_qn .= $shop_yibu_tb_last_year;
                ...
                $list_yb_yhd_qn .= $shop_yibu_yhd_last_year;
                ...
                $list_yb_qn .= $shop_yibu_last_year;

            }
        }

        $shop_yibu_tm_this_year = $v1['shop_yibu_tm'];//依布-天猫数据
        $shop_yibu_tb_this_year = $v1['shop_yibu_tb'];//依布-淘宝数据
        $shop_yibu_yhd_this_year = $v1['shop_yibu_yhd'];//依布-一号店数据
        $shop_yibu_this_year = $shop_yibu_tm_this_year + $shop_yibu_tb_this_year + $shop_yibu_yhd_this_year;//依布-数据
...
//返回 总销售额 依布 去年
    ...
    switch ($shop) {
        case "天猫":
            ...
            $result .= '"list_sale_info_day_qn":{"all":' . $shop_yibu_last_year . ',"tm":' . $shop_yibu_tm_last_year . ',"tb":' . $shop_yibu_tb_last_year . ',"yhd":' . $shop_yibu_yhd_last_year . '},';
            $result .= '"list":[{"name":"依布天猫","data":[' . $list_yb_tm_jn . ']},{"name":"依布天猫去年","data":[' . $list_yb_tm_qn . ']}]';
            break;
        case "淘宝":
            ...
            $result .= '"list_sale_info_day_jn":{"all":' . $shop_yibu_this_year . ',"tm":' . $shop_yibu_tm_this_year . ',"tb":' . $shop_yibu_tb_this_year . ',"yhd":' . $shop_yibu_yhd_this_year . '},';
            $result .= '"list_sale_info_day_qn":{"all":' . $shop_yibu_last_year . ',"tm":' . $shop_yibu_tm_last_year . ',"tb":' . $shop_yibu_tb_last_year . ',"yhd":' . $shop_yibu_yhd_last_year . '},';
            $result .= '"list":[{"name":"依布淘宝","data":[' . $list_yb_tb_jn . ']},{"name":"依布淘宝去年","data":[' . $list_yb_tb_qn . ']}]';
            break;
        case "一号店":
            ...
            $result .= '"list_sale_info_day_jn":{"all":' . $shop_yibu_this_year . ',"tm":' . $shop_yibu_tm_this_year . ',"tb":' . $shop_yibu_tb_this_year . ',"yhd":' . $shop_yibu_yhd_this_year . '},';
            $result .= '"list_sale_info_day_qn":{"all":' . $shop_yibu_last_year . ',"tm":' . $shop_yibu_tm_last_year . ',"tb":' . $shop_yibu_tb_last_year . ',"yhd":' . $shop_yibu_yhd_last_year . '},';
            $result .= '"list":[{"name":"依布一号店","data":[' . $list_yb_yhd_jn . ']},{"name":"依布一号店去年","data":[' . $list_yb_yhd_qn . ']}]';
            break;
        default:

            //默认返回 依布 整个部门 全年销售额
            $result .= '"data_title":["依布今年","依布去年"],';
            $result .= '"data_time":[' . $re_data_time . '],';
            $result .= '"list_sale_info_jn":{"all":' . $all_sales_yb_jn . ',"tm":' . $all_sales_yb_tm_jn . ',"tb":' . $all_sales_yb_tb_jn . ',"yhd":' . $all_sales_yb_yhd_jn . '},';
            $result .= '"list_sale_info_qn":{"all":' . $all_sales_yb_qn . ',"tm":' . $all_sales_yb_tm_qn . ',"tb":' . $all_sales_yb_tb_qn . ',"yhd":' . $all_sales_yb_yhd_qn . '},';
            $result .= '"list_sale_info_day_jn":{"all":' . $shop_yibu_this_year . ',"tm":' . $shop_yibu_tm_this_year . ',"tb":' . $shop_yibu_tb_this_year . ',"yhd":' . $shop_yibu_yhd_this_year . '},';
            $result .= '"list_sale_info_day_qn":{"all":' . $shop_yibu_last_year . ',"tm":' . $shop_yibu_tm_last_year . ',"tb":' . $shop_yibu_tb_last_year . ',"yhd":' . $shop_yibu_yhd_last_year . '},';
            $result .= '"list":[{"name":"依布今年","data":[' . $list_yb_jn . ']},{"name":"依布去年","data":[' . $list_yb_qn . ']}]';
    }
