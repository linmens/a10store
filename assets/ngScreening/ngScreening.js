...
(function () {
    ...
    function _resizeAll() {
        setTimeout(function () {
            ...
            for (var i = 0; i < screenings.length; i++) {
                if (screenings[i].screening_resize) {
                    screenings[i].screening_resize()
                }
            }
        })
    }
    ...
    m.directive('screening', function () {
        return {
            ...
            transclude: true,
            require: '^ngScreening',
            template: '<div class="screening" data-index="{{$index}}">\
